...
- KTD7. **Serialize reporter mutations within the current concurrency contract.** A static reporter concurrency group uses `cancel-in-progress: false`. GitHub Actions permits one active and one pending run for the group; a newer pending run can replace the older pending run. This is not a durable queue: operation idempotency, visible displacement diagnostics, manual reruns, and GitHub state reconciliation remain mandatory.
...
- **Approach:**
  ...
  - Give reporter only `contents: write` and `issues: write`, no PR/discussion permission or agent credentials. Apply one static concurrency group with `cancel-in-progress: false`; GitHub permits one active and one pending reporter run, and a newer pending run can replace the older pending run, so displacement must remain visible and manually rerunnable.
...
- **Test scenarios:**
  ...
  - **Concurrency:** Scheduled and manual reporters use one non-cancelling concurrency group with one active and one pending run; a newer pending run can replace the older pending run, and displacement remains visible and manually rerunnable after checking the same operation key.
...
- **Approach:**
  - Document triggers, write modes, permission boundaries, artifact versus release retention, ledger ownership, retry/checkpoint behavior, human closure override, diagnostics, pending-run replacement behavior, and emergency disable procedure.
...
| Reporter run is displaced or issue state changes during a run | GitHub permits one active and one pending run, and a newer pending run can replace the older pending run; expose displacement for manual retry, re-read immediately before mutation, and retain operation idempotency. |
...
| Scheduled activation | `enabled` | Both cron slots preserve generic behavior and perform clean no-op or correct idempotent mutation | Pending-run displacement not surfaced for retry, duplicate issue/asset/comment, required-check regression, or failure represented as clean |
