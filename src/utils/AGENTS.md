...
| `analytics.ts` | Typed Umami catalog/adapter plus `/privacy` privacy metadata; no queues, session state, error tracking, or performance tracking |
...
- **Analytics**: `analytics.ts` validates bounded categorical events, applies DNT/path privacy policy, adapts calls to Umami, and supplies the event metadata rendered by `/privacy`; it does not queue, retry, retain session state, or collect error/performance data
